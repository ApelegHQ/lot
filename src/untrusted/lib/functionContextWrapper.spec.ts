...
describe('functionContextWrapper', () => {
	it('should wrap script without preamble correctly', () => {
		const script = 'console.log("test")';

		const result = functionContextWrapper(script);

		for (const expr of ['with', 'use strict']) {
	...
	it('should wrap script with preamble correctly', () => {
		const script = 'console.log("test")';
		const preamble = 'var test=true;';

		const result = functionContextWrapper(script, preamble);
